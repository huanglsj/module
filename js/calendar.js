    ...
    var mdater = {
      ...
      refreshView: function() {
        if (this.input.hasClass('input-group')) {
          ...
          if(appcan.locStorage.getVal("setExpDetails")!=null)
            {
                objson = eval('(' + appcan.locStorage.getVal("setExpDetails")+ ')');
                var min="20"+objson.begin;
                initVal=min;
            }
            else if(appcan.locStorage.getVal("beginDate")!=null)
            {
                initVal=appcan.locStorage.getVal("beginDate");
            }
            else
            {
               initVal=this.input.val() 
            }           
        }
        var date = null;
        if (initVal) {
      ...
      saveValueToInput: function() {
        ...
        if (_this.input.hasClass('input-group')) {
         ...
         /* var airDate = document.getElementById("air_date");
          var startTime = new Date(airDate.innerHTML); */
          if(inDate.getTime() < (new Date(option.minDate)).getTime()){
              //appcan.window.openToast("预定日期不能小于出发日期",2000);
              _this._hidePanel();
              return new Date(option.minDate);
          }
          else if(option.maxDate != null && inDate.getTime() > (new Date(option.maxDate)).getTime()){
              //appcan.window.openToast("预定日期不能大于返回日期",2000);
              _this._hidePanel();
              return new Date(option.maxDate);
          }
          _this.input.val(_this.value.year + '-' + monthValue + '-' + dateValue);
          _this.input.trigger('input');
        }
        if(_this.input.find("div")){
          _this.input.find("span").html("<span class='air_date'>"+_this.value.year + '-' + monthValue + '-' + dateValue+'</span><span class="tx_red">'+getDayInWeek(inDate)+'</span>');  
        } 
        else if(_this.input.find("span")){
          _this.input.find("span").html("<span class='air_date'>"+_this.value.year + '-' + monthValue + '-' + dateValue);  
        }
        _this._hidePanel();
        if(appcan.locStorage.getVal("openFlightSearchResult")=="true")
            {
               $("#today").html(_this.value.year + '-' + monthValue + '-' + dateValue);
               appcan.locStorage.setVal("airDate",_this.value.year + '-' + monthValue + '-' + dateValue);
               $("#planeTicketlist").html("");
               var planeTicketListViewInstance = new planeTicketListView();
                planeTicketListViewInstance.load();
                planeTicketListViewInstance.collection.on('sort',todayPrice);     
            }
        return _this.value.year + '-' + monthValue + '-' + dateValue;
      ...
      afterShowPanel: function(mask, panel) {
        ...
        panel.delegates({
          ...
          '.md_ok': function() {                      
            if(appcan.locStorage.getVal("openFlightSearchResult")=="true")
            {
               $("#today").html(_this.saveValueToInput());
               appcan.locStorage.setVal("airDate",_this.saveValueToInput());
               $("#planeTicketlist").html("");
               var planeTicketListViewInstance = new planeTicketListView();
                planeTicketListViewInstance.load();  
            }

		...
		search: function(keyword) {
			...
			var checkGroup = function(currentIndex, last) {
				if (itemCount >= currentIndex - groupIndex - (last ? 0 : 1)) {
					selectorBuffer.push(classSelector('indexed-list-inner li') + ':nth-of-type(' + (groupIndex + 1) + ')');
			...
			liArray.forEach(function(item) {
				...
				if (item.classList.contains($.className('indexed-list-group'))) {
					...
					var value = (item.getAttribute('data-pinyinname') || '').toLowerCase();
					var tags = (item.getAttribute('data-city') || '').toLowerCase();
					if (keyword && text.indexOf(keyword) < 0 &&
						value.indexOf(keyword) < 0 &&
						tags.indexOf(keyword) < 0) {
						selectorBuffer.push(classSelector('indexed-list-inner li') + ':nth-of-type(' + (currentIndex + 1) + ')');
						itemCount++;
					}

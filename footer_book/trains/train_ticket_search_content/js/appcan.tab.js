    ...
    f.prototype = {
        focusAnim : function(b, c) {
            ...
            if (d.option.hasAnim) {
                ...
                d.focus = d.focus || a('<div class="focus utra sc-text-hint"></div>'), d.focus.css("width", f), d.focus.css("-webkit-transform", "translateX(" + b * f + "px)"), c && c.append(d.focus)
            }
        },
        focusText : function(b) {
            ...
            if (!(c.option.hasAnim && c.option.hasIcon && c.option.hasLabel)) {
                var d = a(".item", c.ele);
                d.length && (d.removeClass("sc-text-active "), a(d[b]).addClass("sc-text-hint "))
            }
        },
        set : function(b, c) {
            var d = this;
